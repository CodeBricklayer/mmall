...
import com.tanp.mmall.common.TokenCache;
import com.tanp.mmall.dao.UserMapper;
import com.tanp.mmall.pojo.User;
import com.tanp.mmall.service.IUserService;
import com.tanp.mmall.util.MD5Util;
...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> checkValid(String str, String type) {
        if (StringUtils.isNoneBlank(type)) {
            ...
            if (Const.EMAIL.equals(type)) {
                ...
                if (resultCount > 0) {
                    return ServerResponse.createByErrorMessgae("email 已存在");
                }
            }
        } else {
    ...
    public ServerResponse<String> checkAnswer(String username, String question, String answer) {
        ...
        if (resultCount > 0) {
            ...
            String forgetToken = UUID.randomUUID().toString() + "-" + MD5Util.MD5EncodeUtf8(String.valueOf(new Date().hashCode()));
    ...
    public ServerResponse<User> updateInformation(User user) {
        //username是不能被更新的
        //email也要进行一个校验，校验新的email是不是已经存在，并且存在email如果相同的话，不能是我们当前这个用户的
        int resultCount = userMapper.checkEmailByUserId(user.getEmail(), user.getId());
        if (resultCount > 0) {
            return ServerResponse.createByErrorMessgae("email已存在，请更换email再尝试更新");
        }
        ...
        return ServerResponse.createByErrorMessgae("更新个人信息失败");
    }
